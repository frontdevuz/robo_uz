...
// Cart effect demo
const cartFx = document.getElementById('cartFx');
function burst() {
  if (!cartFx) return;
  for (let i = 0; i < 18; i++) {
    const orb = document.createElement('div');
    orb.className = 'orb';
    const x0 = (Math.random() * 100 - 50) + 'vw';
    const y0 = (Math.random() * 60 - 30) + 'vh';
    const x1 = (Math.random() * 100 - 50) + 'vw';
    const y1 = (Math.random() * 80 - 40) + 'vh';
    orb.style.setProperty('--x0', x0);
    orb.style.setProperty('--y0', y0);
    orb.style.setProperty('--x1', x1);
    orb.style.setProperty('--y1', y1);
    cartFx.appendChild(orb);
